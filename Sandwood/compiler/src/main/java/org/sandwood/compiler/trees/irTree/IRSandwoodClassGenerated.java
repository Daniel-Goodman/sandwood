...
public class IRSandwoodClassGenerated {
    ...
    private <A extends Variable<A>> void addGetterAndSetter(FieldDesc<A> f) {
        ...
        if(f.fieldType.getter) {
            ...
            if(f.varDesc.equals(modelProbabilityName))
                ...
                get = IRTree.returnFunction(override ? Visibility.PUBLIC : Visibility.DEFAULT, f.varDesc.type,
                        FunctionName.getterName(f.varDesc.name), new ArgDesc[0], body, override,
                        "Getter for " + f.varDesc + ".");
        ...
        if(f.fieldType.setter) {
            ...
            IRVoidFunction set = IRTree.voidFunction(override ? Visibility.PUBLIC : Visibility.DEFAULT,
                    FunctionName.setterName(f.varDesc.name), args, body, override, "Setter for " + f.varDesc + ".");
    ...
    public TransSandwoodClassGenerated toTransformationTree() {
        ...
        return new TransSandwoodClassGenerated(target, name, packageName, modelCode, transFunctions, classFieldTrees,
                scratchFieldTrees, transGettersAndSetters);
    }
}
